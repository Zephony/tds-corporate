...
export default function SmsAddView(props: SmsAddViewProps) {
    ...
    // Calculate character count when SMS content changes
    useEffect(() => {
        const smsContent = props?.updateData?.['sms_content'] || props?.updateData?.['body'] || ''
        const characterCount = smsContent.length
        const currentCount = parseInt(props?.updateData?.['character_count'] || '0', 10)
        
        // Only update if the count has changed to avoid infinite loops
        if (characterCount !== currentCount && props.updateOnChange) {
    ...
    }, [props?.updateData?.['sms_content'], props?.updateData?.['body']])
    ...
    return (
        <div className='edit-wrapper sms-add-view'>
            {/* Template Type */}
            <div className='edit-section'>
                <Input
                    label='Template Type'
                    name='template_type'
                    value={props?.updateData?.['template_type'] || ''}
            ...
            {/* Active, Agent Send, and Marketing Dropdowns */}
            <div className='edit-section dropdown-row'>
                ...
                <SelectField
                    ...
                    value={props?.updateData?.['agent_send'] || ''}
                    onChange={props.updateOnChange}
                    className='input-wrapper'
                    viewMode='edit'
                />
            ...
            <div className='edit-section'>
                <SelectField
                    ...
                    value={props?.updateData?.['marketing'] || ''}
                    onChange={props.updateOnChange}
                    className='input-wrapper'
                    viewMode='edit'
                />
            ...
            <div className='edit-section'>
                <Input
                    ...
                    name='character_count'
                    value={props?.updateData?.['character_count'] || '0'}
                    onChange={props.updateOnChange}
                    className='input-wrapper'
                    type='number'
                    placeholder='0'
            ...
            <div className='edit-section-email'>
                <Input
                    label='SMS Content'
                    name='sms_content'
                    value={props?.updateData?.['sms_content'] || props?.updateData?.['body'] || ''}
                    onChange={props.updateOnChange}
                    className='input-wrapper'
                    type='textarea'
                    placeholder='Content here'
                />
                {renderTagLinks('sms_content')}
            </div>
        </div>
    )
}
